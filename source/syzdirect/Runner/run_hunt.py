...
def _analysis_quarantine_dir(bc_dir):
    # Keep quarantined files outside the analysis tree so interface_generator
    # does not recursively load them again on the next retry.
...
def parse_interface_generator_failures(log_path, bc_dir):
    ...
    # The shell may print "Segmentation fault" after tee exits, so the log can
    # end with only the last in-flight module. Use that module as a fallback.
    if last_module:
...
def run_interface_generator_with_retries(bc_dir, iface_dir, output_sig, max_attempts=6):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        sh(f"cd {Q(iface_dir)} && "
           f"{Q(INTERFACE_GENERATOR)} --verbose-level=4 {Q(bc_dir)} 2>&1 | tee log",
...
def ensure_commit_available(src_dir, commit, fetch_ref):
    ...
    sh(f"cd {Q(src_dir)} && git fetch --depth=1 origin {Q(fetch_ref)}", check=False)
    if fetch_ref != commit:
        sh(f"cd {Q(src_dir)} && git fetch --depth=1 origin {Q(commit)}", check=False)
    if subprocess.run(verify_cmd, cwd=src_dir, check=False,
                      stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode != 0:
...
def normalize_syscall_name(name, context_file=""):
    ...
    context_file = (context_file or "").lower()
    ...
    if base == "io_uring_register":
        preferred = "io_uring_register$IORING_REGISTER_EVENTFD" if "eventfd" in context_file \
    ...
    def step3_analyze_kernel(self):
        ...
        for dp in self.datapoints:
            ...
            if not os.path.exists(sig):
                ok = run_interface_generator_with_retries(bc, iface_dir, sig)
                if not ok:
...
class NewCVEPipeline:
    ...
    def _create_fuzzinps(self):
        ...
        syscalls = normalize_callfile_entries(syscalls, self.file_path)
    ...
    def _ensure_fuzz_inputs(self):
        ...
        for xidx in tfmap.keys():
            ...
            with open(x_callfile) as f:
                entries = json.load(f)
            normalized = normalize_callfile_entries(entries, self.file_path)
